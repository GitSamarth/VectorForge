...
int main() {
    ...
    svr.Post("/doc/ask", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        /*Groundedness gate: refuse to generate an answer when retrieval confidence is low.
         docDB.search() returns cosine distance (0 = identical, ~1+ = unrelated), so
         hits[0].first is the distance of the single best-matching chunk.
        Threshold 0.5 is a starting heuristic, not derived from evaluation — tune this
         against real queries/documents rather than trusting it as-is.
        //
         Without this gate, a weak/irrelevant top match still gets sent to the LLM,
         which will often answer anyway using its own general knowledge and give no
         signal that it did — the original prompt-only approach relied on the model
         self-reporting insufficient context, which LLMs do unreliably. This check
        makes the "documents-only" guarantee enforced by code, not by instruction.*/
    if (hits.empty() || hits[0].first > 0.5f) {
    // No chunks retrieved, or best match too weak to trust as grounding —
    // short-circuit before calling the LLM at all. Saves a slow (10-30s)
    // generation call and avoids an ungrounded hallucination reaching the user.
        
        res.set_content("{\"answer\":\"I couldn't find enough relevant information in the retrieved documents to answer your question.\","
                         "\"contexts\":[]}", "application/json");
        return;
    }
       ...
       std::ostringstream ctx;
        ...
        std::string prompt =
            ...
            "Question: " + question + "\n\n"
            "Answer:";*/  
            //this prompt can ctreate hallucnation because it allows the model to use its own knowledge if the context is insufficient. To strictly enforce that the model only uses the provided context, we can modify the prompt as follows
